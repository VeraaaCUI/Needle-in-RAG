...
# NOTE: transformers' private helper `_crop_past_key_values` moved/changed across versions.
# We provide a backward/forward-compatible wrapper to avoid hard dependency on a specific version.
try:
    from transformers.generation.utils import _crop_past_key_values as _hf_crop_past_key_values  # type: ignore
except Exception:  # pragma: no cover
    _hf_crop_past_key_values = None  # type: ignore

def _crop_past_key_values(model, past_key_values, max_length: int):
    """Best-effort cropping of KV cache across transformers versions.

    If we cannot crop safely, returns the cache unchanged.
    """
    if past_key_values is None:
        return None

    # Preferred: delegate to HF private helper when available.
    ...
    # Newer cache objects may implement `.crop(max_length)`.
    try:
    ...
    # Fallback: classic tuple-of-layer KV format.
    try:
        if isinstance(past_key_values, (tuple, list)):
            ...
            for layer in past_key_values:
                if isinstance(layer, (tuple, list)) and len(layer) >= 2:
                    k, v = layer[0], layer[1]
                    # Typical shapes: [batch, heads, seq_len, head_dim]
                    if hasattr(k, "shape") and len(getattr(k, "shape", ())) >= 3:
                        seq_dim = -2
                        if k.shape[seq_dim] > max_length:
                            k = k[..., :max_length, :]
                    if hasattr(v, "shape") and len(getattr(v, "shape", ())) >= 3:
                        seq_dim = -2
                        if v.shape[seq_dim] > max_length:
                            v = v[..., :max_length, :]
                    cropped.append((k, v, *layer[2:]))
                else:
...
class DecodingAgg(RRAG):
    ...
    def preprocess_input(self,data_item):
        ...
        # batched version 
        input_dict_draft = self.llm.tokenizer(prompt_list_draft, return_tensors="pt", padding=True).to(self.llm.device)
        # there is some issues when certifying the model with the batched version, will fix this later
        input_ids_draft = input_dict_draft.input_ids.to(self.llm.device)
        attention_mask_draft = input_dict_draft.attention_mask.to(self.llm.device)
        ...
        total_probability = torch.ones(input_ids_draft.shape[0]).to(self.llm.device)
        ...
        input_ids = input_dict.input_ids.to(self.llm.device)
        attention_mask = input_dict.attention_mask.to(self.llm.device)
    ...
    def query(self, data_item, corruption_size=1):
        ...
        stop_token_ids = [LongTensor(x).to(self.llm.device) for x in stop_token_ids]
